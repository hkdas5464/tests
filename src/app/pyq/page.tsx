...
import { animals, countryStateData, CurrentAffairs, EnglishData, jpsc2021, jpscData, Mathsdata, Reasoning, SSC2024 } from "./data";
...
function SSCCGL() {
  ...
  const [jpscstates, setJpsctates] = useState<{ link: string; label: string }[]>([]);
  ...
  const handleCountryJpsc = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const countryKey = event.target.value;
    const country = jpscData.find((c) => c.countryKey === countryKey);
    setJpsctates(country ? country.states : []);
    setIsActive(true); // Toggle between true and false
  };
          ...
          <div className="flex w-full flex-col">
              <div className="w-full flex flex-row flex-wrap gap-4">
                <Select
                  label="Select Year"
                  className="max-w-xs"
                  placeholder='JPSC 2021'
                  onChange={handleCountryJpsc}
                >
                  {jpscData.map((country) => (
                    <SelectItem key={country.countryKey} value={country.countryKey}>
                      {country.countryName}
                    </SelectItem>
                  ))}
